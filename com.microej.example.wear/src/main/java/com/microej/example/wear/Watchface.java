...
/**
 * A widget that represents an analog watchface.
 *
 * <p>
 ...
 */
public class Watchface extends Widget implements Animation {
	...
	protected void onShown() {
		super.onShown();
		// the watchface is now shown on the display, start the hands animation
		startAnimation();
	}
	...
	protected void onHidden() {
		super.onHidden();
		// ensure that no animation is running when the watchface is not shown on the display
		stopAnimation();
	}

	@Override
	public boolean tick(long currentTimeMillis) {
		// update the display at each step of the animation
		requestRender();
		return true;
	}
	...
	private static float computeHourAngle(long time) {
		...
		if (hour >= TimeHelper.NOON) {
			hour -= TimeHelper.NOON;
		}
	 ...
	 */
	private static float computeSecondAngle(long time) {
	...
	public boolean handleEvent(int event) {
		if (Event.getType(event) == Command.EVENT_TYPE) {
			...
			WatchExample.showApplicationList();
			return true;
		}
